...
def create_launchers(os_name):
    ...
    if os_name == "Windows":
        ...
        with open("start.sh", "w") as f:
            ...
            f.write('cd "$DIR"\n')
            f.write("\n")
            f.write("# Auto-update: git pull if clean repo\n")
            f.write('if git -C "$DIR" rev-parse --git-dir &>/dev/null; then\n')
            f.write('    if git -C "$DIR" diff --quiet 2>/dev/null; then\n')
            f.write('        echo "📡 Проверка обновлений..."\n')
            f.write('        PULL_OUTPUT=$(git -C "$DIR" pull --ff-only 2>&1)\n')
            f.write("        if [ $? -eq 0 ] && [ \"$PULL_OUTPUT\" != \"Already up to date.\" ]; then\n")
            f.write('            echo "   ✅ Обновлено"\n')
            f.write("        else\n")
            f.write('            echo "   ✅ Актуальная версия"\n')
            f.write("        fi\n")
            f.write('        echo ""\n')
            f.write("    else\n")
            f.write('        echo "⚠️  Есть локальные изменения — git pull пропущен"\n')
            f.write('        echo ""\n')
            f.write("    fi\n")
            f.write("fi\n")
            f.write("\n")
